...
function Navigation($page,$pages) {
   $n='';
    ...
    if($pages>1){
		...
		//if ($end<$pages - 10) $n.=' ... <a href="'.HOME.LANG.'/prosmotr-gruzov-nayti-gruz-poisk-poputnyih-gruzov-predlozheniya-gruzov-888.php?export='.$_GET['export'].'&import='.$_GET['import'].'&page='.$pages.'"  class="btn btn-link" style="margin-right:0.5em;">1</a>';
        $n.='</div>';
...
return $n;
  ...
  <button class="btn btn-warning" type="button"  style="float:right;background-image: -webkit-linear-gradient(top, #ffffff, #e6e6e6);color:#333;border-color: #adadad;background-image: -moz-linear-gradient(top, #ffffff, #e6e6e6);min-width:10%" onclick="SearchBirja()"><? echo Lang('Search',LANG)?></button>
	...
	if(mysqli_num_rows($result)==0) echo $query.'<br>'. mysqli_error($db);
...

	{
...

		{
			...
			<td>'.$fio.'<br>'.$phone.'<br><a href="'.HOME.'cargo/info.php?language='.LANG.'&id='.$cargo['id'].'" ';?>onclick='return popup(this, "Popup")'<?php echo' target="_blank">'.$more.'</a></td>
...

{
	...
	$.post(home_url+'cargo/birja_gruzaPost.php', {country_from:From,country_to:To,transport_type:Type},function(data){
