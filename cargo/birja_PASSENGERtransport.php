...
if ($page>10) $n.='<a href="'.HOME.LANG.'/passenger_transport.php?export='.$_GET['export'].'&import='.$_GET['import'].'&page=1"  class="btn btn-link" style="margin-right:0.5em;">1</a> ...';
        ...
        for($i = $start; $i <= $end; $i++){
            ...
            $n.='<a href="'.HOME.LANG.'/passenger_transport.php'.(($i != 1) ? '?page='.$i : '').'"'.(($page == $i) ? ' class="btn btn-primary" ' : ' class="btn btn-link"').'  style="margin-right:0.5em;">'.$i.'</a>';
        ...
        if($end < $pages) {
            ...
            $n.='<a href="'.HOME.LANG.'/passenger_transport.php?page='.$pages.'" class="btn btn-link" style="margin-left:0.5em;">'.$pages.'</a>';
...
return $n;
	...
	$query = "SELECT * FROM `movers_order_passengers`  WHERE `hidden`='0' ";
...
<div class="form-group">
  ...
  <button class="btn btn-warning" type="button"  style="float:right;background-image: -webkit-linear-gradient(top, #ffffff, #e6e6e6);color:#333;border-color: #adadad;background-image: -moz-linear-gradient(top, #ffffff, #e6e6e6);min-width:10%" onclick="SearchBirja()"><? echo Lang('Search',LANG)?></button>
...

	{
		...
		if (LANG=='ru') $t4 = 'Пассажиров';



		if (LANG=='ro') $t4 = 'Passageri';



		if (LANG=='en') $t4 = 'Passengers';



		
		if (LANG=='ru') $t5 = 'Дата отправки';

		if (LANG=='ro') $t5 = 'Data încarcării';

		if (LANG=='en') $t5 = 'Loading date';
	...
	echo'</div>';
...

		{
			...
			<td style="width:15%">'.$cargo['passenger_number'].'</td>
			<td style="width:15%">'.$cargo['date'].'</td>
			<td>'.mb_substr($fio,0,50,"UTF-8").'<br>'.$phone.'<br><a href="'.HOME.'cargo/info_passengertransport.php?language='.LANG.'&id='.$cargo['id'].'" ';?>onclick='return popup(this, "Popup")'<?php echo' target="_blank">'.$more.'</a></td>
			</tr>';
...

{
	...
	$.post(home_url+'cargo/birja_passengertransportPost.php', {country_from:From,country_to:To,transport_type:Type},function(data){
