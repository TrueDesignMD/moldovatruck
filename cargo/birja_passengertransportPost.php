...
return $n;
	...
	if (!empty($_POST['transport_type'])) $query.=" AND `type`='".htmlspecialchars($_POST['transport_type'],ENT_QUOTES)."'";
	
	if (!empty($_GET['export'])) {$import = htmlspecialchars($_GET['import'],ENT_QUOTES);}
	if (!empty($_GET['import'])) {$export = htmlspecialchars($_GET['export'],ENT_QUOTES);}
