...
function Navigation($page,$pages) {
   $n='';
	...
	if ($page>10) $n.='<a href="'.HOME.LANG.'/train_cargo.php?export='.$_GET['export'].'&import='.$_GET['import'].'&page=1"  class="btn btn-link" style="margin-right:0.5em;">1</a> ...';
      

	  for($i = $start; $i <= $end; $i++){
            $n.='<a href="'.HOME.LANG.'/train_cargo.php?export='.$_GET['export'].'&import='.$_GET['import'].(($i != 1) ? '&page='.$i : '').'"'.(($page == $i) ? ' class="btn btn-primary" ' : ' class="btn btn-link"').'   style="margin-right:0.5em;">'.$i.'</a>';
        }
        if($end < $pages) {
           if($end != ($pages - 1)) $n.='...';
            $n.='<a href="'.HOME.LANG.'/train_cargo.php?export='.$_GET['export'].'&import='.$_GET['import'].'&page='.$pages.'" class="btn btn-link" style="margin-left:0.5em;">'.$pages.'</a>';
        }
        $n.='</div>';
    }
return $n;
  ...
  <button class="btn btn-warning" type="button"  style="float:right;background-image: -webkit-linear-gradient(top, #ffffff, #e6e6e6);color:#333;border-color: #adadad;background-image: -moz-linear-gradient(top, #ffffff, #e6e6e6);min-width:10%" onclick="SearchBirja()"><? echo Lang('Search',LANG)?></button>
...

	{
...

		{
			...
			<td>'.$fio.'<br>'.$phone.'<br><a href="'.HOME.'cargo/train_info.php?language='.LANG.'&id='.$cargo['id'].'" ';?>onclick='return popup(this, "Popup")'<?php echo' target="_blank">'.$more.'</a></td>
...
if(LANG=='ru') $tt = 'Добавить груз для Ж.Д. перевозок';

if(LANG=='ro') $tt = 'Adaugarea marfei pentru transportări feroviare';

if(LANG=='en') $tt = 'Add cargo for rail transportations';
...
echo '<div class="modal-content">
			...
			include (dirname(__FILE__)."/addTrainCargoFrom.php");
			echo'</div>
			</div>
		  </div>';?></div></div>
...
if(LANG=='ru') $tt = 'Добавить транспорт для Ж.Д. перевозок';

if(LANG=='ro') $tt = 'Adaugarea transportului pentru transportări feroviare';

if(LANG=='en') $tt = 'Add transport for rail transportations';
...
echo '<div class="modal-content">
			...
			<div class="modal-body">';
			include (dirname(__FILE__)."/addTrainTransportFrom.php");
			echo'</div>
			</div>
		  </div>';?></div></div>
...

{
	...
	$.post(home_url+'cargo/birja_traingruzaPost.php', {country_from:From,country_to:To},function(data){
