...
if ($page>10) $n.='<a href="'.HOME.LANG.'/prosmotr-gruzov-nayti-gruz-poisk-poputnyih-gruzov-predlozheniya-gruzov-888.php?export='.$_POST['country_from'].'&import='.$_POST['country_to'].'&page=1"  class="btn btn-link" style="margin-right:0.5em;">1</a> ...';
       
        for($i = $start; $i <= $end; $i++){



            $n.='<a href="'.HOME.LANG.'/prosmotr-gruzov-nayti-gruz-poisk-poputnyih-gruzov-predlozheniya-gruzov-888.php?export='.$_POST['country_from'].'&import='.$_POST['country_to'].(($i != 1) ? '&page='.$i : '').'"'.(($page == $i) ? ' class="btn btn-primary" ' : ' class="btn btn-link"').' style="margin-right:0.5em;">'.$i.'</a>';
        ...
        if($end < $pages) {
            ...
            $n.='<a href="'.HOME.LANG.'/prosmotr-gruzov-nayti-gruz-poisk-poputnyih-gruzov-predlozheniya-gruzov-888.php?export='.$_POST['country_from'].'&import='.$_POST['country_to'].'&page='.$pages.'" class="btn btn-link" style="margin-left:0.5em;">'.$pages.'</a>';
...
return $n;
	...
	if (!empty($start)) $query.=" ORDER BY `id` DESC LIMIT ".$start.",50";

	else $query.=' ORDER BY `id` DESC LIMIT 50';
