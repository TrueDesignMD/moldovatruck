...
if ($page>10) $n.='<a href="'.HOME.LANG.'/train_transport.php?export='.$_GET['export'].'&import='.$_GET['import'].'&page=1"  class="btn btn-link" style="margin-right:0.5em;">1</a> ...';
        ...
        for($i = $start; $i <= $end; $i++){
            ...
            $n.='<a href="'.HOME.LANG.'/train_transport.php'.(($i != 1) ? '?page='.$i : '').'"'.(($page == $i) ? ' class="btn btn-primary" ' : ' class="btn btn-link"').'  style="margin-right:0.5em;">'.$i.'</a>';
        ...
        if($end < $pages) {
            ...
            $n.='<a href="'.HOME.LANG.'/train_transport.php?page='.$pages.'" class="btn btn-link" style="margin-left:0.5em;">'.$pages.'</a>';
...
echo '<div style="float:right;margin-top:1em">'.$page_.' '.$_GET['page'].' '.$from_.' '.$PagesCount.' </div>';
...

		{
			...
			<td>'.mb_substr($fio,0,50,"UTF-8").'<br>'.$phone.'<br><a href="'.HOME.'cargo/info_traintransport.php?language='.LANG.'&id='.$cargo['id'].'" ';?>onclick='return popup(this, "Popup")'<?php echo' target="_blank">'.$more.'</a></td>
			</tr>';
...
if(LANG=='en') $tt = 'Add cargo for rails transportations';
...
if(LANG=='en') $tt = 'Add transport for rails transportations';
...

{
	...
	$.post(home_url+'cargo/birja_traintransportPost.php', {country_from:From,country_to:To},function(data){
