...
function Navigation($page,$pages) {
    ...
    if($pages>1){
		...
		if ($page>10) $n.='<a href="'.HOME.LANG.'/post_transport.php?export='.$_GET['export'].'&import='.$_GET['import'].'&page=1"  class="btn btn-link" style="margin-right:0.5em;">1</a> ...';
        ...
        for($i = $start; $i <= $end; $i++){
            ...
            $n.='<a href="'.HOME.LANG.'/post_transport.php'.(($i != 1) ? '?page='.$i : '').'"'.(($page == $i) ? ' class="btn btn-primary" ' : ' class="btn btn-link"').'  style="margin-right:0.5em;">'.$i.'</a>';
        ...
        if($end < $pages) {
            ...
            $n.='<a href="'.HOME.LANG.'/post_transport.php?page='.$pages.'" class="btn btn-link" style="margin-left:0.5em;">'.$pages.'</a>';
...
return $n;
	...
	$query = "SELECT * FROM `movers_order_post` WHERE `hidden`='0' ";
	if (!empty($_POST['country_from'])) $query.=" and `export`='".htmlspecialchars($_POST['country_from'],ENT_QUOTES)."'";
	if (!empty($_POST['country_from'])&&!empty($_POST['country_to'])) $query.=" AND `import`='".htmlspecialchars($_POST['country_to'],ENT_QUOTES)."'";
	...
	if (!empty($_POST['transport_type']) and (empty($_POST['country_to']) and empty($_POST['country_from']))) $query.=" WHERE `type`='".htmlspecialchars($_POST['transport_type'],ENT_QUOTES)."'";


	if (!empty($_GET['export'])) {$import = htmlspecialchars($_GET['export'],ENT_QUOTES);}
	if (!empty($_GET['import'])) {$export = htmlspecialchars($_GET['import'],ENT_QUOTES);}
	if (!empty($_GET['export'])) $query.=" WHERE `import`='".htmlspecialchars($_GET['export'],ENT_QUOTES)."'";



	if (!empty($_GET['import'])&&!empty($_GET['export'])) $query.=" AND `export`='".htmlspecialchars($_GET['import'],ENT_QUOTES)."'";


	if (!empty($_GET['import'])&&empty($_GET['export'])) $query.=" WHERE `export`='".htmlspecialchars($_GET['import'],ENT_QUOTES)."'";
...
echo '<div style="float:right;margin-top:1em">'.$page_.' '.$_GET['page'].' '.$from_.' '.$PagesCount.' </div>';
...

		{
			...
			$countryExport = mysqli_fetch_array($getCountryExport);
			$getTransportType = mysqli_query($db,"SELECT `transport_type_$mosConfig_lang` FROM `transport_type_post` WHERE `id`='$cargo[type]'");
			$TransportType = mysqli_fetch_array($getTransportType);
			$getVolume = mysqli_query($db,"SELECT `cargo_volume_$mosConfig_lang` FROM `cargo_volume_post` WHERE `id`='$cargo[volume]'");
			...
			<td>'.mb_substr($fio,0,50,"UTF-8").'<br>'.$phone.'<br><a href="'.HOME.'cargo/info_posttransport.php?language='.LANG.'&id='.$cargo['id'].'" ';?>onclick='return popup(this, "Popup")'<?php echo' target="_blank">'.$more.'</a></td>
			</tr>';
