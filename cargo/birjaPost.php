...
if ($page>10) $n.='<a href="'.HOME.LANG.'/prosmotr-gruzov-nayti-gruz-poisk-poputnyih-gruzov-predlozheniya-gruzov-888.php?export='.$_GET['export'].'&import='.$_GET['import'].'&page=1"  class="btn btn-link" style="margin-right:0.5em;">1</a> ...';
        for($i = $start; $i <= $end; $i++){
            $n.='<a href="'.HOME.LANG.'/prosmotr-gruzov-nayti-gruz-poisk-poputnyih-gruzov-predlozheniya-gruzov-888.php?export='.$_GET['export'].'&import='.$_GET['import'].(($i != 1) ? '&page='.$i : '').'"'.(($page == $i) ? ' class="btn btn-primary" ' : ' class="btn btn-link"').' style="margin-right:0.5em;">'.$i.'</a>';
        }
        if($end < $pages) {
           if($end != ($pages - 1)) $n.='...';
            $n.='<a href="'.HOME.LANG.'/prosmotr-gruzov-nayti-gruz-poisk-poputnyih-gruzov-predlozheniya-gruzov-888.php?export='.$_GET['export'].'&import='.$_GET['import'].'&page='.$pages.'" class="btn btn-link" style="margin-left:0.5em;">'.$pages.'</a>';
        }
        $n.='</div>';
    }
return $n;
	...
	if (!empty($_POST['country_from'])) {$export = htmlspecialchars($_POST['country_from'],ENT_QUOTES);}
	if (!empty($_POST['country_to'])) {$import = htmlspecialchars($_POST['country_to'],ENT_QUOTES);}
	
	
	if (!empty($_GET['country_from'])) $query.=" AND `export`='".htmlspecialchars($_GET['country_from'],ENT_QUOTES)."'";
	if (!empty($_GET['country_to'])) $query.=" AND `import`='".htmlspecialchars($_GET['country_to'],ENT_QUOTES)."'";
	...
	}else echo '<br><b>Ничего не найдено</b>';
	?>
	<script>
	$(function () {
		...
		$('#table thead th:eq('+k+')').css('width',($('#table tbody tr:eq(0) td:eq('+k+')').width()+10.5)+'px');
