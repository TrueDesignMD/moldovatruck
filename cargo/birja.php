...
function Navigation($page,$pages) {
    ...
    if($pages>1){
        ...
        for($i = $start; $i <= $end; $i++){
            ...
            $n.='<a href="'.HOME.LANG.'/poisk-transporta-nayti-svobodnyiy-transport-poisk-poputnogo-transporta-577.php'.(($i != 1) ? '?page='.$i : '').'"'.(($page == $i) ? ' class="btn btn-primary" ' : ' class="btn btn-link"').' style="margin-right:0.5em;">'.$i.'</a>';
        ...
        if($end < $pages) {
            ...
            $n.='<a href="'.HOME.LANG.'/poisk-transporta-nayti-svobodnyiy-transport-poisk-poputnogo-transporta-577.php?page='.$pages.'" class="btn btn-link" style="margin-left:0.5em;">'.$pages.'</a>';
...
return $n;
	...
	$AutoID = "2,3,4,5,7,8,9,10,11,12,13,14,16,17,18,19,20,21,22,23,26,27,28,29,32,35,37,45,''";
	$query.=" WHERE `type` IN (".$AutoID.") ";
	...
	if (!empty($_GET['export'])) $query.=" AND `import`='".htmlspecialchars($_GET['import'],ENT_QUOTES)."'";
	if (!empty($_GET['import'])&&!empty($_GET['export'])) $query.=" AND `export`='".htmlspecialchars($_GET['export'],ENT_QUOTES)."'";
	if (!empty($_GET['import'])&&empty($_GET['export'])) $query.=" AND `export`='".htmlspecialchars($_GET['export'],ENT_QUOTES)."'";
	...
	$query.=" AND `hidden`='0' ORDER BY `id` DESC";
 ...
 <div class="form-group">
  ...
  <div class="form-group">
    ...
    <select class="form-control" id="country_to" style="padding: 0px;"  name="country_to"><option value=""><?php if (LANG=='ru') echo'Выберите страну разгрузки</option><option value="">Все страны';?><?php if (LANG=='ro') echo'Selectați țara</option><option value="">Toate țările';?><?php if (LANG=='en') echo'Select unloading country</option><option value="">All countries';?></option><?php echo $countries2; ?></select>
  ...
  <button class="btn btn-warning" type="button"  style="float:right;background-image: -webkit-linear-gradient(top, #ffffff, #e6e6e6);color:#333;border-color: #adadad;background-image: -moz-linear-gradient(top, #ffffff, #e6e6e6);min-width:10%" onclick="SearchBirja()"><? echo Lang('Search',LANG)?></button>
	...
	echo'</div>';
...

		{
			...
			<td>'.mb_substr($fio,0,50,"UTF-8").'<br>'.$phone.'<br><a href="'.HOME.'cargo/info_transport.php?language='.LANG.'&id='.$cargo['id'].'" ';?>onclick='return popup(this, "Popup")'<?php echo' target="_blank">'.$more.'</a></td>
			</tr>';
		...
		echo '<div style="float:right">'.$page_.' '.$_GET['page'].' '.$from_.' '.$PagesCount.' </div>';
		echo Navigation(htmlspecialchars($_GET['page'],ENT_QUOTES),$PagesCount);
...
if(LANG=='ru') $tt = 'Добавить груз для автоперевозок';

if(LANG=='ro') $tt = 'Adaugarea marfei pentru autocar';

if(LANG=='en') $tt = 'Add cargo for auto';
...
echo '<div class="modal-content">
			...
			include (dirname(__FILE__)."/calculate_cost_of_transportation_cargo.php");
			echo'</div>
			</div>
		  </div>';?></div></div>
...
if(LANG=='ru') $tt = 'Добавить транспорт для автоперевозок';

if(LANG=='ro') $tt = 'Adaugarea transportului pentru autocamioane';

if(LANG=='en') $tt = 'Add transport for autotransportations';
...
echo '<div class="modal-content">
			...
			<div class="modal-body">';
			include (dirname(__FILE__)."/left_online_form_transport.php");
			echo'</div>
			</div>
		  </div>';?></div></div>
...

{
	...
	To = $('#country_to').val();
	Type = $('#transport_type').val();
	$('#result').html(loading);



	$.post(home_url+'cargo/birjaPost.php', {country_from:From,country_to:To,transport_type:Type},function(data){
