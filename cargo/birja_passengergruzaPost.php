...
function Navigation($page,$pages) {
   $n='';
    ...
    if($pages>1){
        ...
        if ($page>10) $n.='<a href="'.HOME.LANG.'/passenger_cargo.php?export='.$_GET['export'].'&import='.$_GET['import'].'&page=1"  class="btn btn-link" style="margin-right:0.5em;">1</a> ...';
       
	   for($i = $start; $i <= $end; $i++){
            $n.='<a href="'.HOME.LANG.'/passenger_cargo.php?export='.$_GET['export'].'&import='.$_GET['import'].(($i != 1) ? '&page='.$i : '').'"'.(($page == $i) ? ' class="btn btn-primary" ' : ' class="btn btn-link"').'   style="margin-right:0.5em;">'.$i.'</a>';
        }
        if($end < $pages) {
           if($end != ($pages - 1)) $n.='...';
            $n.='<a href="'.HOME.LANG.'/passenger_cargo.php?export='.$_GET['export'].'&import='.$_GET['import'].'&page='.$pages.'" class="btn btn-link" style="margin-left:0.5em;">'.$pages.'</a>';
        }
        $n.='</div>';
    }
return $n;
	...
	$query = "SELECT * FROM `movers_cargo_passengers`  WHERE `hidden`='0'";
	if (!empty($_POST['country_from'])) $query.=" AND `export`='".htmlspecialchars($_POST['country_from'],ENT_QUOTES)."'";
	if (!empty($_POST['country_to'])&&!empty($_POST['country_from'])) $query.=" AND `import`='".htmlspecialchars($_POST['country_to'],ENT_QUOTES)."'";
	if (!empty($_POST['country_to'])&&empty($_POST['country_from'])) $query.=" AND `import`='".htmlspecialchars($_POST['country_to'],ENT_QUOTES)."'";
	if (!empty($_POST['transport_type']) and (!empty($_POST['country_to']) or !empty($_POST['country_from']))) $query.=" AND `type`='".htmlspecialchars($_POST['transport_type'],ENT_QUOTES)."'";
	if (!empty($_POST['transport_type']) and (empty($_POST['country_to']) and empty($_POST['country_from']))) $query.=" AND `type`='".htmlspecialchars($_POST['transport_type'],ENT_QUOTES)."'";
	...
	if (!empty($_GET['export'])) $query.=" WHERE `export`='".htmlspecialchars($_GET['export'],ENT_QUOTES)."'";
	if (!empty($_GET['import'])&&!empty($_GET['export'])) $query.=" AND `import`='".htmlspecialchars($_GET['import'],ENT_QUOTES)."'";
	if (!empty($_GET['import'])&&empty($_GET['export'])) $query.=" WHERE `import`='".htmlspecialchars($_GET['import'],ENT_QUOTES)."'";
	...
	if (LANG=='ru')$title = '<br><b>Всего в вашем направлении посылки ';
...

	{
		...
		if (LANG=='ru') $t6 = 'Кол-во пассажиров';

		if (LANG=='ro') $t6 = 'Passageri';

		if (LANG=='en') $t6 = 'Count passengers';
...
echo '<div style="float:right;margin-top:1em">'.$page_.' '.$_GET['page'].' '.$from_.' '.$PagesCount.' </div>';
...

		{
			...
			$getContainer = mysqli_query($db,"SELECT `container_type_$mosConfig_lang` FROM `container_type_rails` WHERE `id`='$cargo[container_type]'");
			$Conteiner = mysqli_fetch_array($getContainer);

			$getTransportType = mysqli_query($db,"SELECT `transport_type_$mosConfig_lang` FROM `transport_type_passengers` WHERE `id`='$cargo[type]'");
			...
			<td  style="width:15%">'.$cargo['order_date'].'</td>
			...
			echo'</td>
			<td style="text-align:center;width:10%">'.$cargo['passenger_number'].'</td>

			<td>'.$fio.'<br>'.$phone.'<br><a href="'.HOME.'cargo/passenger_info.php?language='.LANG.'&id='.$cargo['id'].'" ';?>onclick='return popup(this, "Popup")'<?php echo' target="_blank">'.$more.'</a></td>
