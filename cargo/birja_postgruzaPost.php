...
function Navigation($page,$pages) {
   $n='';
    ...
    if($pages>1){
        ...
        if ($page>10) $n.='<a href="'.HOME.LANG.'/post_cargo.php?export='.$_GET['export'].'&import='.$_GET['import'].'&page=1"  class="btn btn-link" style="margin-right:0.5em;">1</a> ...';
      
		for($i = $start; $i <= $end; $i++){
            $n.='<a href="'.HOME.LANG.'/post_cargo.php?export='.$_GET['export'].'&import='.$_GET['import'].(($i != 1) ? '&page='.$i : '').'"'.(($page == $i) ? ' class="btn btn-primary" ' : ' class="btn btn-link"').'   style="margin-right:0.5em;">'.$i.'</a>';
        }
        if($end < $pages) {
           if($end != ($pages - 1)) $n.='...';
            $n.='<a href="'.HOME.LANG.'/post_cargo.php?export='.$_GET['export'].'&import='.$_GET['import'].'&page='.$pages.'" class="btn btn-link" style="margin-left:0.5em;">'.$pages.'</a>';
        }
        $n.='</div>';
    }
return $n;
	...
	$query = "SELECT * FROM `movers_cargo_post` WHERE `hidden`='0'";
	if (!empty($_POST['country_from'])) $query.=" AND `export`='".htmlspecialchars($_POST['country_from'],ENT_QUOTES)."'";
	if (!empty($_POST['country_from'])&&!empty($_POST['country_to'])) $query.=" AND `export`='".htmlspecialchars($_POST['country_from'],ENT_QUOTES)."'";
...

	{
...

		{
			...
			$getGruz = mysqli_query($db,"SELECT `cargo_type_$mosConfig_lang` FROM `cargo_type_post` WHERE `id`='$cargo[name]'");
			...
			<td>'.$fio.'<br>'.$phone.'<br><a href="'.HOME.'cargo/post_info.php?language='.LANG.'&id='.$cargo['id'].'" ';?>onclick='return popup(this, "Popup")'<?php echo' target="_blank">'.$more.'</a></td>
