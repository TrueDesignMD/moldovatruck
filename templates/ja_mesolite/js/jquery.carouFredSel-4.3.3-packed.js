...
(function($) {
	...
	$.fn.carouFredSel = function(o) {
		...
		$cfs.bind_events = function() {
			...
			$cfs.bind('pause.cfs', function(e, g) {
				e.stopPropagation();
			...
			$cfs.bind('play.cfs', function(e, d, f, g) {
				e.stopPropagation();
				$cfs.trigger('pause');
			...
			$cfs.bind('prev.cfs next.cfs', function(e, o, n, c) {
				e.stopPropagation();
				if (isPaused == 'stopped' || $cfs.is(':animated') || $cfs.is(':hidden')) {
			...
			$cfs.bind('slide_prev.cfs', function(e, sO, nI) {
				e.stopPropagation();
			...
			$cfs.bind('slide_next.cfs', function(e, sO, nI) {
				e.stopPropagation();
				if (opts.variableVisible) {
					opts.items.oldVisible = opts.items.visible;
					var a_itm = getItems($cfs);
			...
			$cfs.bind('slideTo.cfs', function(e, num, dev, org, obj) {
				e.stopPropagation();
			...
			$cfs.bind('insertItem.cfs', function(e, itm, num, org, dev) {
				e.stopPropagation();
			...
			$cfs.bind('removeItem.cfs', function(e, num, org, dev) {
				e.stopPropagation();
				if (typeof num == 'undefined' || num == 'end') {
			...
			$cfs.bind('currentPosition.cfs', function(e, fn) {
				e.stopPropagation();
			...
			$cfs.bind('currentPage.cfs', function(e, fn) {
				e.stopPropagation();
			...
			$cfs.bind('currentVisible.cfs', function(e, fn) {
				e.stopPropagation();
			...
			$cfs.bind('isPaused.cfs', function(e, fn) {
				e.stopPropagation();
				if (typeof fn == 'function') fn.call($tt0, isPaused);
				return isPaused;
			});

			$cfs.bind('configuration.cfs', function(e, a, b, reInit) {
				e.stopPropagation();
			...
			$cfs.bind('linkAnchors.cfs', function(e, $con, sel) {
				e.stopPropagation();
			...
			$cfs.bind('updatePageStatus.cfs', function(e, build, sizes) {
				e.stopPropagation();
			...
			$cfs.bind('destroy.cfs', function(e, orgOrder) {
				e.stopPropagation();
				if (orgOrder) {
		...
		$cfs.bind_buttons = function() {
			...
			if (opts.auto.pauseOnHover) {
				$wrp.bind('mouseenter.cfs', function() { $cfs.trigger('pause'); });
				$wrp.bind('mouseleave.cfs', function() { $cfs.trigger('play');	});
			}
			if (opts.prev.button) {
				opts.prev.button.bind('click.cfs', function(e) {
					e.preventDefault();
					$cfs.trigger('prev');
				});
				if (opts.prev.pauseOnHover) {
					opts.prev.button.bind('mouseenter.cfs', function() { $cfs.trigger('pause'); });
					opts.prev.button.bind('mouseleave.cfs', function() { $cfs.trigger('play');	});
				}
			}
			if (opts.next.button) {
				opts.next.button.bind('click.cfs', function(e) {
					e.preventDefault();
					$cfs.trigger('next');
				});
				if (opts.next.pauseOnHover) {
					opts.next.button.bind('mouseenter.cfs', function() { $cfs.trigger('pause'); });
					opts.next.button.bind('mouseleave.cfs', function() { $cfs.trigger('play');	});
				}
			}
			if ($.fn.mousewheel) {
			...
			if (opts.pagination.container) {
				if (opts.pagination.pauseOnHover) {
					opts.pagination.container.bind('mouseenter.cfs', function() { $cfs.trigger('pause'); });
					opts.pagination.container.bind('mouseleave.cfs', function() { $cfs.trigger('play');	});
				}
			}
			if (opts.next.key || opts.prev.key) {
				$(document).bind('keyup.cfs', function(e) {
					var k = e.keyCode;
					if (k == opts.next.key)	{
						e.preventDefault();
						$cfs.trigger('next');
		...
		$cfs.unbind_buttons = function() {
			$(document).unbind('.cfs');
			$wrp.unbind('.cfs');
			if (opts.prev.button) 			opts.prev.button.unbind('.cfs');
			if (opts.next.button) 			opts.next.button.unbind('.cfs');
			if (opts.pagination.container)	opts.pagination.container.unbind('.cfs');
			showNavi(opts, 'hide');
