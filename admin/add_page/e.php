...
if (!empty($_POST))
{	
	...
	if (!empty($title3)&&!empty($message3)&&$insertRU)
	{		if(empty($metatitle3)) $metatitle3 = $title3;		
		...
		$URL = 'en/'.strtolower(translit($title));		$insertEN = mysqli_query($CONNECTION,"INSERT INTO `".DB_PREFIX."pages`(`pageName`,`URL`,`Content`,`Active`,`langAbb`,`MetaDescription`,`MetaTags`,`MetaTitle`) VALUES ('$title3', '$URL', '$message3', '1','en','$metadesc3','$metakey3','$metatitle3')");		if (!$insertEN) echo mysqli_error($CONNECTION);
	}
