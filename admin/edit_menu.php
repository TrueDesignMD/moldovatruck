...
<?php
	...
	{
		$getParents = mysqli_query($CONNECTION,"SELECT `Parent`,`menuName` FROM `".DB_PREFIX."menu` WHERE `menuTypeId`='$arr[1]' AND `langAbb`='ru' AND `Active`='1' GROUP BY `Parent` ORDER BY `Order`");
		if (mysqli_num_rows($getParents)>0)
		{
			...
			do
			{
				...
				$getMenu = mysqli_query($CONNECTION,"SELECT * FROM `".DB_PREFIX."menu` WHERE `menuTypeId`='$arr[1]' AND `Parent`='$Parents[Parent]' AND `langAbb`='ru' AND `Active`='1' ORDER BY `Order`");
