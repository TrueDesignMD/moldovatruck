...
if (!empty($_POST))
{
	...
	if (!empty($title)&&!empty($message))
	{
		...
		$insertRU = mysqli_query($CONNECTION,"UPDATE `".DB_PREFIX."pages` SET  `pageName`='$title',`Content`='$message',`MetaDescription`='$metadesc',`MetaTags`='$metakey' WHERE `pageId`='$ID'");
	...
	if (!empty($title2)&&!empty($message2)&&$insertRU)
	{
		...
		if (mysqli_num_rows($getURL)==0){
			$insertRO = mysqli_query($CONNECTION,"INSERT INTO `".DB_PREFIX."pages` (`pageName`,`URL`,`Content`,`Active`,`langAbb`,`MetaDescription`,`MetaTags`) VALUES ('$title2', '$URLn', '$message2','1','ro','$metadesc2','$metakey2')");
		}
		else{
				$URL = mysqli_fetch_array($getURL);
				$insertRO = mysqli_query($CONNECTION,"UPDATE `".DB_PREFIX."pages` SET `pageName`='$title2',`Content`='$message2',`MetaDescription`='$metadesc2',`MetaTags`='$metakey2' WHERE `pageId`='".$URL['pageId']."'");
			}
	...
	if (!empty($title3)&&!empty($message3)&&$insertRU)
	{
		...
		if (mysqli_num_rows($getURLEN)==0){
			$insertRO = mysqli_query($CONNECTION,"INSERT INTO `".DB_PREFIX."pages` (`pageName`,`URL`,`Content`,`Active`,`langAbb`,`MetaDescription`,`MetaTags`) VALUES ('$title3', '$URLn', '$message3','1','en','$metadesc3','$metakey3')");
		}
		else{
				$URLEN = mysqli_fetch_array($getURLEN);
				$insertRO = mysqli_query($CONNECTION,"UPDATE `".DB_PREFIX."pages` SET  `pageName`='$title3',`Content`='$message3',`MetaDescription`='$metadesc3',`MetaTags`='$metakey3' WHERE `pageId`='".$URLEN['pageId']."'");
			}
		if (!$insertRO) echo mysqli_error($CONNECTION);
	}
...
<form action="<?php echo $admin_page;?>edit_page" method="post" enctype="multipart/form-data">
