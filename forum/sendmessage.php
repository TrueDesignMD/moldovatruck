...
if(isset($_POST))
{
	...
	if(mb_strlen($name,'UTF-8')>3&&mb_strlen($message,'UTF-8')>15&&strlen($email)>10&&strlen($phone)>5)
	{
		$body = '
...
<body style="width:100%;text-align:center;padding:0px;margin:0px;">
	...
	<div style="margin-top:15px;color:white;padding:10px;border:1px solid #ccc;padding-top:30px;padding-bottom:30px;background-color:#F62E24;font-size:18px;text-align:center;">Оповещения</div>
...
$body.='<div style="margin-top:15px;padding:10px;padding-top:30px;padding-bottom:30px;font-size:16px;text-align:center;"><i>'.$message.'</i></div>
	<p align=Center style="color:#000;margin-top:25px;">При возникновении вопросов пишите jeniabuianov@gmail.com</p></div>
		...
		$send = send_mail('s1.magicnet.md',465,'ssl',true,'Moldovatruck оповещения','notifications@moldovatruck.md','S(N=w)&-[lXl','Moldova Truck','moversauto@gmail.com','UTF-8','Обратная связь форума ',$body);
		
		if(is_bool($send) and $send==true) echo 'Сообщение отправленно'; else echo $send;
	}
	else echo 'Длина текста должна быть не менее 15 символов, Email не менее 10 символов, телефон не менее 5 символов'; 
